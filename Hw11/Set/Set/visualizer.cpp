...
void Visualizer::save(std::string way) {
	
	m_image.saveToFile(way);

}

void Visualizer::run(float c_1, float c_2)
{
	...
	while (m_window.isOpen())
	{
		...
		update(c_1, c_2);
		render();
		//save("screenshots\\1.png");
	}
	
}

void Visualizer::screenshot(float c_1, float c_2) {
	review();
	update(c_1, c_2);
	render();
}
...
void Visualizer::update(float c_1, float c_2)
{
	
	if (m_need_redraw)
	{
		m_calculator.calculate(m_view.center(), m_view.width(), m_view.height(),c_1, c_2);
	}
	...
	if (m_need_redraw)
	{
		for (auto y = 0U; y < m_calculator.height(); ++y)
		{
			for (auto x = 0U; x < m_calculator.width(); ++x)
			{
				...
				/*std::complex < float > zz(x, y);
				if (x == 900 && y == 900) {
					std::cout << m_calculator.field()[x][y];
					
				}
				if (x < 505 && x > 480 && y < 505 && y > 500) {
					//std::cout << m_calculator.field()[x][y] << std::endl;
					m_image.setPixel(x, y, sf::Color::Red);
					continue;
				}
				float val = m_calculator.max_iterations() - static_cast<float>(iterations) / m_calculator.max_iterations();
				m_image.setPixel(x, y, sf::Color(
					255 * val, 255 * (1 - val), 255 * (std::abs(zz) / 1414)));*/
				if (iterations == 0U || iterations == m_calculator.max_iterations())
				{
					m_image.setPixel(x, y, sf::Color::Black);
					//float val = static_cast<float>(iterations) / m_calculator.max_iterations();
					//m_image.setPixel(x, y, sf::Color(
						//255 * val, 255 * (1 - val), 255 * (std::abs(zz) / 1414)));
				}
				else
				{
					...
					//m_image.setPixel(x, y, sf::Color(255, 0, 255 * (std::abs(zz) / 1414)));
				}
			}
		}
